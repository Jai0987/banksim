import { OAuth2Server, MutableToken }  from 'oauth2-mock-server';
import {parse} from 'basic-auth';
import * as fs from 'fs';
import { ClientConfig, ConfigData, UserConfig } from './config-types.js';
...
const config: [ConfigData] = JSON.parse(jsonString);
...
server.service.on('beforeTokenSigning', (token: MutableToken, req) => {
...
function buildClientToken(token: MutableToken, req: any){
...
function buildUserToken(token: MutableToken, req: any){
...
function sanitizeClient(client: ConfigData) {
